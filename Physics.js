...
class WorldView {
  ...
  getTranslationConst() {
    return [
      canvas.width / 2 - this.pixelsPerMeter * this.center.x,
  ...
  transformCoord(worldCoord) {
     ...
     */
    return new Vector(
      this.translationConst[0] + pixelsPerMeter * worldCoord.x,
...
function elasticCircleCollision(circBody1, circBody2) {
  ...
  new_vr1 =
    ((circBody1.mass - circBody2.mass) * vr1 + 2 * circBody2.mass * vr2) /
    (circBody1.mass + circBody2.mass);
  new_vr2 =
    ((circBody2.mass - circBody1.mass) * vr2 + 2 * circBody1.mass * vr1) /
    (circBody1.mass + circBody2.mass);
  // Compute total new velocities by adding radial and tangential components
  circBody1.vel = unitRadial
    .scalarMult(new_vr1)
    .add(unitTangential.scalarMult(vt1));
  circBody2.vel = unitRadial
    .scalarMult(new_vr2)
    .add(unitTangential.scalarMult(vt2));
}
...
function QuadTree(level, bounds) {
  ...
  this.split = function () {
    ...
    nodes[0] = new QuadTree(
      this.level + 1,
      new Rect(x, y, midWidth, midHeight)
    );
    nodes[1] = new QuadTree(
      this.level + 1,
      new Rect(x + midWidth, y, midWidth, midHeight)
    );
    nodes[2] = new QuadTree(
      this.level + 1,
      new Rect(x, y + midHeight, midWidth, midHeight)
    );
    nodes[3] = new QuadTree(
      this.level + 1,
      new Rect(x + midWidth, y + midHeight, midWidth, midHeight)
    );
  };
  ...
  this.getIndex = function (rectObject) {
    ...
    if (
      rectObject.y < center.y &&
      rectObject.y + rectObject.height < center.y
    ) {
      if (
        rectObject.x > bounds.x &&
        rectObject.x + rectObject.width < center.x
      ) {
        index = 0;
      } else if (
        rectObject.x > center.x &&
        rectObject.x + rectObject.width < bounds.x + bounds.width
      ) {
        index = 1;
      }
    } else if (
      rectObject.y > center.y &&
      rectObject.y + rectObject.height < bounds.y + bounds.height
    ) {
      if (
        rectObject.x > bounds.x &&
        rectObject.x + rectObject.width < center.x
...
function getTotalKineticEnergy(bodies) {
  var KE = 0;
  for (var i = 0; i < bodies.length; i++) {
    KE += 0.5 * bodies[i].mass * bodies[i].vel.getLength() ** 2;
  }
  return KE;
}
